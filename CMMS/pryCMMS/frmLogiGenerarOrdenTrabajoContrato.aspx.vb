...
Public Class frmLogiGenerarOrdenTrabajoContrato
    ...
    Sub LlenarData()
        ...
        Else
            'Dim dateValueIni As DateTime = DateTime.Parse(row.Cells(3).Text)

            'row.Cells(3).Text = dateValueIni.ToString("yyyy-MM-dd")
            'txtFechaInicioPlanificadaMantenimientoOrdenTrabajo.Text = row.Cells(3).Text
            Dim dateValueIni As DateTime
            Dim formato As String = "dd/MM/yyyy" ' Asegúrate de que este es el formato correcto
            If DateTime.TryParseExact(row.Cells(4).Text, formato, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, dateValueIni) Then
            ...
            Else
                ...
                txtFechaInicioPlanificadaMantenimientoOrdenTrabajo.Text = "Fecha inválida"
            End If
        End If

        If (String.IsNullOrEmpty(row.Cells(18).Text) Or row.Cells(18).Text.Equals("&nbsp;")) Then
            txtFechaTerminoPlanificadaMantenimientoOrdenTrabajo.Text = Now.ToString("yyyy-MM-dd")
        Else
            'Dim dateValueTer As DateTime = DateTime.Parse(row.Cells(16).Text)
            'row.Cells(16).Text = dateValueTer.ToString("yyyy-MM-dd")
            'txtFechaTerminoPlanificadaMantenimientoOrdenTrabajo.Text = row.Cells(16).Text
            Dim dateValueTer As DateTime
            Dim formato As String = "dd/MM/yyyy" ' Asegúrate de que este es el formato correcto
            If DateTime.TryParseExact(row.Cells(18).Text, formato, System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, dateValueTer) Then
                row.Cells(4).Text = dateValueTer.ToString("yyyy-MM-dd")
                txtFechaTerminoPlanificadaMantenimientoOrdenTrabajo.Text = row.Cells(4).Text
            Else
                ' Manejar el error si la fecha no es válida
                txtFechaTerminoPlanificadaMantenimientoOrdenTrabajo.Text = "Fecha inválida"
            End If
        End If
        ...
        'JMUG: 18/07/2025 cboTipoMantenimientoMantenimientoOrdenTrabajo.SelectedValue = row.Cells(20).Text
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Me.IsPostBack = False Then 'Si la pagina no a hecho Postback; es el ida y vuelta de la pagina web al servidor web
            ...
            'Dim strValorContrato As String = ""
            'Dim srtUsuarioFiltro As String = ""
            'If Len(Session("IdContratoUsuario")) > 2 Then strValorContrato = Session("IdContratoUsuario") Else strValorContrato = ""
            'If Session("IdPerfil") = "00015" Then srtUsuarioFiltro = Session("IdUsuario") Else srtUsuarioFiltro = ""

            'SubFiltro = " AND EQU.vContratoReferenciaActualEquipo LIKE '%" & strValorContrato & "%' AND DOC.cIdTipoDocumentoCabeceraOrdenTrabajo + '-'+ DOC.vIdNumeroSerieCabeceraOrdenTrabajo + '-' + DOC.vIdNumeroCorrelativoCabeceraOrdenTrabajo IN (SELECT RECORDTRA.cIdTipoDocumentoCabeceraOrdenTrabajo + '-' + RECORDTRA.vIdNumeroSerieCabeceraOrdenTrabajo + '-' + RECORDTRA.vIdNumeroCorrelativoCabeceraOrdenTrabajo " &
            ...
            '                "USU.cIdUsuario LIKE '%" & srtUsuarioFiltro & "%' AND USU.vIdNroDocumentoIdentidadUsuario = PER.vNumeroDocumentoPersonal " &
            '                "WHERE RECORDTRA.bResponsableRecursosOrdenTrabajo IN ('0', '1') AND RECORDTRA.cIdEmpresa = '" & Session("IdEmpresa") & "')"

            ''Me.grdLista.DataSource = OrdenTrabajoNeg.OrdenTrabajoListaBusqueda("cIdTipoDocumentoCabeceraOrdenTrabajo = 'OT' AND LTRIM(RTRIM(ISNULL(vContratoReferenciaCabeceraOrdenTrabajo, ''))) <> '' AND (cEstadoCabeceraOrdenTrabajo = '" & cboFiltroStatus.SelectedValue & "' OR '*' = '" & cboFiltroStatus.SelectedValue & "') AND " & cboFiltroOrdenTrabajo.SelectedValue, txtBuscarOrdenTrabajo.Text, Session("IdEmpresa"), IIf(Session("IdTipoUsuario") = "A", "", SubFiltro), "1")
            'Me.grdLista.DataSource = OrdenTrabajoNeg.OrdenTrabajoListaBusqueda("cIdTipoDocumentoCabeceraOrdenTrabajo = 'OT' AND LTRIM(RTRIM(ISNULL(vContratoReferenciaCabeceraOrdenTrabajo, ''))) <> '' AND (cEstadoCabeceraOrdenTrabajo = '" & cboFiltroStatus.SelectedValue & "' OR '*' = '" & cboFiltroStatus.SelectedValue & "') AND " & cboFiltroOrdenTrabajo.SelectedValue, txtBuscarOrdenTrabajo.Text, Session("IdEmpresa"), IIf(Session("IdTipoUsuario") = "A", "", SubFiltro), "1")
    ...
    Sub ListarPlantillaCheckListCombo()
        ...
        cboListadoCheckListMantenimientoOrdenTrabajo.Items.Add("SELECCIONE DATO")

        cboListadoCheckListMantenimientoOrdenTrabajo.DataBind()


        If (String.IsNullOrEmpty(row.Cells(19).Text) Or row.Cells(19).Text.Equals("&nbsp;")) Then

        Else
            'cboListadoCheckListMantenimientoOrdenTrabajo.SelectedValue = row.Cells(18).Text
            cboListadoCheckListMantenimientoOrdenTrabajo.SelectedValue = row.Cells(19).Text
        End If
                ...
                If grdLista.Rows.Count > 0 Then
                    If IsNothing(grdLista.SelectedRow) = False Then
                        If IsReference(grdLista.SelectedRow.Cells(1).Text) = True Then
                            ...
                            lnkbtnVerCheckList.Attributes.Add("onclick", "javascript:popupEmitirOrdenTrabajoCheckListReporte('" & Server.HtmlDecode(grdLista.SelectedRow.Cells(0).Text) & "', '" & Server.HtmlDecode(grdLista.SelectedRow.Cells(1).Text) & "', '" & Server.HtmlDecode(grdLista.SelectedRow.Cells(2).Text) & "', '" & Session("IdEmpresa") & "');")
                            Dim lblEstado As Label = TryCast(grdLista.SelectedRow.Cells(15).FindControl("lblEstado"), Label)
    ...
    Private Sub grdLista_RowCreated(sender As Object, e As GridViewRowEventArgs) Handles grdLista.RowCreated
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            For Each td In e.Row.Cells
                ...
                e.Row.Cells.Item(3).HorizontalAlign = HorizontalAlign.Center 'Ver
                ...
                e.Row.Cells.Item(7).HorizontalAlign = HorizontalAlign.Center 'RucCliente
                ...
                e.Row.Cells.Item(15).HorizontalAlign = HorizontalAlign.Left 'Estado
                e.Row.Cells.Item(16).HorizontalAlign = HorizontalAlign.Left 'Estado Registro
                e.Row.Cells.Item(17).HorizontalAlign = HorizontalAlign.Left 'FechaEmision
                e.Row.Cells.Item(18).HorizontalAlign = HorizontalAlign.Left 'FechaTermino
                e.Row.Cells.Item(19).HorizontalAlign = HorizontalAlign.Left 'IdNumeroCabeceraCheckListPlantilla
                'e.Row.Cells.Item(20).HorizontalAlign = HorizontalAlign.Left 'TipoMantenimiento
            Next
        End If
    End Sub
    ...
    Private Sub grdLista_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles grdLista.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            e.Row.Cells(3).Visible = True 'Ver
            ...
            e.Row.Cells(15).Visible = True 'Estado
            e.Row.Cells(16).Visible = False 'Estado Registro
            e.Row.Cells(17).Visible = False 'FechaEmision
            e.Row.Cells(18).Visible = False 'FechaTermino
            e.Row.Cells(19).Visible = False 'IdNumeroCabeceraCheckListPlantilla
            'e.Row.Cells(20).Visible = False 'TipoMantenimiento
        End If
    ...
    Private Sub btnAceptarMantenimientoOrdenTrabajo_Click(sender As Object, e As EventArgs) Handles btnAceptarMantenimientoOrdenTrabajo.Click
        Try
            ...
            Dim splitCN = OrdTra.vContratoReferenciaCabeceraOrdenTrabajo.Split("-")
            ...
            "' WHERE cIdTipoDocumentoCabeceraContrato = '" & splitCN(0) &
            "' AND vIdNumeroSerieCabeceraContrato = '" & splitCN(1) &
            "' AND vIdNumeroCorrelativoCabeceraContrato = '" & splitCN(2) &
            "' AND cIdEmpresa = '" & Session("IdEmpresa") &
            "' AND vOrdenTrabajoReferenciaPlanificacionEquipoContrato = '" & String.Concat("OT-", OrdTra.vIdNumeroSerieCabeceraOrdenTrabajo, "-", OrdTra.vIdNumeroCorrelativoCabeceraOrdenTrabajo) & "'"
    ...
    Private Sub btnAceptarMantenimientoTarea_Click(sender As Object, e As EventArgs) Handles btnAceptarMantenimientoTarea.Click
        Try
            ...
            If Valores(5).ToString = "I" Or Valores(5).ToString = "E" Then
                For i = 0 To Session("CestaSSPersonal").Rows.Count - 1
                    OrdenTrabajoNeg.OrdenTrabajoGetData("INSERT LOGI_TAREACOMPONENTEORDENTRABAJO (nIdNumeroItemTareaComponenteOrdenTrabajo, dFechaInicioTareaComponenteOrdenTrabajo, dFechaFinalTareaComponenteOrdenTrabajo, cIdPersonal, vDescripcionTareaComponenteOrdenTrabajo, cIdTipoDocumentoCabeceraOrdenTrabajo, vIdNumeroSerieCabeceraOrdenTrabajo, vIdNumeroCorrelativoCabeceraOrdenTrabajo, cIdEmpresa, cIdEquipoCabeceraOrdenTrabajo, cIdCatalogoComponenteOrdenTrabajo, cIdJerarquiaCatalogoComponenteOrdenTrabajo, vIdArticuloSAPCabeceraOrdenTrabajo, cIdEquipoComponenteOrdenTrabajo) " &
                                                    ...
                                                    "FROM LOGI_TAREACOMPONENTEORDENTRABAJO WHERE cIdTipoDocumentoCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(0).Text).Trim) & "' AND vIdNumeroSerieCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(1).Text).Trim) & "' AND vIdNumeroCorrelativoCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(2).Text).Trim) & "' AND cIdEmpresa = '" & Session("IdEmpresa") & "' AND cIdEquipoCabeceraOrdenTrabajo = '" & Valores(3).ToString & "' AND cIdCatalogoComponenteOrdenTrabajo = '" & Valores(0).ToString & "' AND cIdJerarquiaCatalogoComponenteOrdenTrabajo = '" & Valores(1).ToString & "' AND vIdArticuloSAPCabeceraOrdenTrabajo = '" & Valores(2).ToString & "')" & ", " &
                                                    "'" & strFechaHoraAtencion & "', NULL, '" & Session("CestaSSPersonal").Rows(i)("Codigo").ToString.Trim & "', '" & UCase(txtDescripcionMantenimientoTarea.Text.Trim) & "', '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(0).Text).Trim) & "', '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(1).Text).Trim) & "', '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(2).Text).Trim) & "', '" & Session("IdEmpresa") & "', '" & Valores(3).ToString & "', '" & Valores(0).ToString & "', '" & Valores(1).ToString & "', '" & Valores(2).ToString & "', '" & Valores(4).ToString & "')")
                ...
                OrdenTrabajoNeg.OrdenTrabajoGetData("UPDATE LOGI_COMPONENTEORDENTRABAJO SET cEstadoComponenteOrdenTrabajo = 'R' " & IIf(Valores(5).ToString = "I", ", dFechaInicioActividadComponenteOrdenTrabajo = '" & strFechaHoraAtencion & "' ", "") &
                                                    "WHERE cIdTipoDocumentoCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(0).Text).Trim) & "' AND vIdNumeroSerieCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(1).Text).Trim) & "' AND vIdNumeroCorrelativoCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(2).Text).Trim) & "' " &
            ...
            ElseIf Valores(5).ToString = "F" Or Valores(5).ToString = "P" Then
                OrdenTrabajoNeg.OrdenTrabajoGetData("UPDATE LOGI_TAREACOMPONENTEORDENTRABAJO SET dFechaFinalTareaComponenteOrdenTrabajo = '" & strFechaHoraAtencion & "' " &
                                                "WHERE cIdTipoDocumentoCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(0).Text).Trim) & "' AND vIdNumeroSerieCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(1).Text).Trim) & "' AND vIdNumeroCorrelativoCabeceraOrdenTrabajo = '" & (Server.HtmlDecode(grdLista.SelectedRow.Cells(2).Text).Trim) & "' AND cIdEmpresa = '" & Session("IdEmpresa") & "' AND cIdEquipoCabeceraOrdenTrabajo = '" & Valores(3).ToString & "' AND cIdCatalogoComponenteOrdenTrabajo = '" & Valores(0).ToString & "' AND cIdJerarquiaCatalogoComponenteOrdenTrabajo = '" & Valores(1).ToString & "' AND vIdArticuloSAPCabeceraOrdenTrabajo = '" & Valores(2).ToString & "' AND dFechaFinalTareaComponenteOrdenTrabajo IS NULL")
        ...
        Try
            ...
            If grdLista IsNot Nothing Then
                If grdLista.Rows.Count > 0 Then
                    If IsNothing(grdLista.SelectedRow) = False Then
                        If IsReference(grdLista.SelectedRow.Cells(1).Text) = True Then
                            MyValidator.ErrorMessage = "Orden de Trabajo por contrato Terminada"
                        End If
                    Else
                        Throw New Exception("Debe de seleccionar un item.")
                    End If
    ...
    Private Sub btnSiFirmarOrdenTrabajo_Click(sender As Object, e As EventArgs) Handles btnSiFirmarOrdenTrabajo.Click
        Try
            ...
            ElseIf Session("IdPerfil") = "00015" Or Session("IdPerfil") = "00014" Then
